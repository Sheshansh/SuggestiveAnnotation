...
tf.app.flags.DEFINE_integer('num_examples', 79,
                            """Number of examples to run.""")
tf.app.flags.DEFINE_boolean('run_once', True,
                            """Whether to run eval only once.""")


def eval_once(saver, dice_op, summary_writer, summary_op, s_fuse, images, labels, i_paths, s_paths):
    """Run Eval once.
    Args:
        saver: Saver.
    ...
    with tf.Session() as sess:
        ...
        try:
            ...
            while step < num_iter and not coord.should_stop():
                s_dice,i_path,s_fuse_out = sess.run([dice_op,i_paths,s_fuse])
                im0 = s_fuse_out[0,:,:,0]
                im1 = s_fuse_out[0,:,:,1]
                image = (im1>im0)*128
...
def evaluate():
    ...
    with tf.Graph().as_default() as g:
        ...
        s_fuse = mainutils.inference_bottleneck(images, train=False)
        ...
        while True:
            eval_once(saver, dice_op, summary_writer, summary_op, s_fuse, images, labels, i_paths, s_paths)
            if FLAGS.run_once:
                break
